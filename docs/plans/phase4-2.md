# Phase 4.2: Discourse API Client
...
- Phase 4.1 complete (exporter database schema)
- Discourse instance with admin API key

## Overview

The Discourse API client provides methods for:

1. **Categories** - Create and manage categories
2. **Topics** - Create topics (threads) with backdating
3. **Posts** - Create and update posts
4. **Users** - Create users with auto-generated passwords
5. **Chat Channels** - Create chat channels (requires Discourse Chat plugin)
6. **Private Messages** - Create private message threads
7. **Uploads** - Upload files and attachments
8. **Rate Limiting** - Handle 429 responses with retry-after

## Tasks

### 1. Core API Client

#### 1.1 Discourse Client Implementation
...
def test_discourse_upload_file(httpx_mock):
...
def test_discourse_create_user(httpx_mock):
    """Test creating a user."""
    httpx_mock.add_response(
        url="https://discourse.example.com/users.json",
        method="POST",
        json={"user_id": 999, "username": "alice"}
    )
...
All requests use API key authentication with headers:
- `Api-Key`: Your Discourse API key
- `Api-Username`: Username to perform actions as (typically "system")

### Rate Limiting

Discourse returns 429 with `Retry-After` header when rate limited. The client raises `DiscourseRateLimitError` which should be caught by retry logic.

### Backdating

Topics and posts support `created_at` parameter for preserving original timestamps from Google Chat.

## Completion Criteria

- [ ] All API methods implemented
- [ ] Rate limiting handled properly
- [ ] Backdating support working
- [ ] File uploads functional
- [ ] User creation working
- [ ] Chat channel support (if plugin available)
- [ ] All tests pass with mocked API

## Next Steps

After Phase 4.2, proceed to Phase 4.3: User Management
