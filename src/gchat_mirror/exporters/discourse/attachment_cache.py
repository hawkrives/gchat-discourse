...
class AttachmentCache:
    ...
    def _upload_attachment(self, attachment_id: str) -> Optional[str]:
        ...
        if not metadata_row:
            logger.error("attachment_metadata_not_found", attachment_id=attachment_id)
            return None
        filename, content_type, size = metadata_row

        # Get file data from attachment storage
        try:
            file_data = self.storage.retrieve_attachment(attachment_id)
        except (ValueError, Exception) as e:
            logger.error("attachment_data_not_found", attachment_id=attachment_id, error=str(e))
            return None
        ...
        try:
            ...
            if not url:
                logger.error(
                    "no_url_in_upload_result", attachment_id=attachment_id, result=upload_result
                )
                return None
            ...
            logger.info("attachment_uploaded", attachment_id=attachment_id, url=url)

            return url

        except Exception as e:
            logger.error("attachment_upload_failed", attachment_id=attachment_id, error=str(e))
            return None
    ...
    def preload_cache(self, attachment_ids: list):
        ...
        logger.debug("attachment_cache_preloaded", count=len(self._memory_cache))
