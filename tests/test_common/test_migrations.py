...
def test_get_applied_migrations_returns_versions(
    empty_db: sqlite3.Connection, tmp_path: Path
) -> None:
    migrations_dir = tmp_path / "migrations"
    migrations_dir.mkdir()
    first = _write_migration(
        migrations_dir, "001_initial", 'conn.execute("CREATE TABLE foo (id INTEGER)")'
    )
...
def test_run_migrations_applies_in_order(tmp_path: Path) -> None:
    ...
    conn = sqlite3.connect(db_path)
    try:
        cursor = conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='alpha'")
