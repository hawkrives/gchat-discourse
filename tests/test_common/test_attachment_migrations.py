...
def test_attachments_db_migration_creates_tables(attachments_db) -> None:
    """Test that migration 002 creates all required tables."""
    # Verify tables exist
    cursor = attachments_db.conn.execute(
        """
...
def test_attachment_inline_table_stores_data(attachments_db) -> None:
    """Test that attachment_inline table can store binary data."""
    # Insert test data
    test_data = b"This is test binary data"
    attachments_db.conn.execute(
        """
...
def test_attachment_chunks_table_stores_chunks(attachments_db) -> None:
...
def test_attachment_chunks_unique_constraint(attachments_db) -> None:
...
def test_attachments_metadata_table_creation(chat_db) -> None:
...
def test_attachments_metadata_stores_data(chat_db) -> None:
...
def test_attachments_foreign_key_constraint(chat_db) -> None:
...
def test_attachments_cascade_delete(chat_db) -> None:
    ...
    chat_db.conn.execute(
        """
        INSERT INTO messages (id, space_id, text)
        VALUES ('msg2', 'space1', 'message with attachment')
        """
    )
    chat_db.conn.execute(
        """
    ...
    # Delete message
    chat_db.conn.execute("DELETE FROM messages WHERE id = ?", ("msg2",))
    chat_db.conn.commit()
...
def test_attachments_indexes_created(chat_db) -> None:
...
def test_attachments_download_tracking(chat_db) -> None:
...
def test_attachments_storage_type_fields(chat_db) -> None:
