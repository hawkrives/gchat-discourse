...
def test_retry_config_exponential_backoff():
    ...
    config = RetryConfig(initial_delay=60, backoff_multiplier=2.0, max_delay=3600)
...
def test_retry_config_max_delay():
    ...
    config = RetryConfig(initial_delay=60, backoff_multiplier=2.0, max_delay=300)
...
def test_retry_config_max_attempts():
    ...
    assert not config.should_retry(5)
    assert not config.should_retry(10)
...
def test_retry_config_permanent_failure_detection():
    ...
    # Permanent failures
    assert config.is_permanent_failure("404 Not Found")
    assert config.is_permanent_failure("403 Forbidden")
    assert config.is_permanent_failure("ValidationError: Invalid username")

    # Temporary failures
    assert not config.is_permanent_failure("429 Rate Limited")
    assert not config.is_permanent_failure("500 Internal Server Error")
    assert not config.is_permanent_failure("Connection timeout")
