...
def test_message_exporter_already_exported(
...
def test_message_exporter_deleted_message(
    discourse_dbs: tuple[sqlite3.Connection, sqlite3.Connection],
) -> None:
    ...
    exporter = MessageExporter(Mock(), state_conn, chat_conn, Mock(), Mock(), Mock(), Mock())
...
def test_message_exporter_message_not_found(
    discourse_dbs: tuple[sqlite3.Connection, sqlite3.Connection],
) -> None:
...
def test_message_exporter_thread_export_fails(
    discourse_dbs: tuple[sqlite3.Connection, sqlite3.Connection],
) -> None:
    ...
    exporter = MessageExporter(
        Mock(), state_conn, chat_conn, Mock(), Mock(), thread_exporter, failed_manager
    )
...
def test_message_exporter_creates_reply_post(
    ...
    thread_exporter = Mock()
    thread_exporter.export_thread.return_value = {"discourse_type": "topic", "discourse_id": 123}
...
def test_message_exporter_with_attachment_cache(
    discourse_dbs: tuple[sqlite3.Connection, sqlite3.Connection], httpx_mock: HTTPXMock
) -> None:
    ...
    # Mock Discourse API
    httpx_mock.add_response(
        url="https://discourse.example.com/posts.json", method="POST", json={"id": 456}
    )
...
def test_message_exporter_handles_blocked_message(
    discourse_dbs: tuple[sqlite3.Connection, sqlite3.Connection],
) -> None:
    ...
    exporter = MessageExporter(
        Mock(), state_conn, chat_conn, Mock(), Mock(), Mock(), failed_manager
    )
...
def test_message_exporter_records_failure_on_thread_export_fail(
    discourse_dbs: tuple[sqlite3.Connection, sqlite3.Connection],
) -> None:
...
def test_message_exporter_exports_edit_history(
    discourse_dbs: tuple[sqlite3.Connection, sqlite3.Connection], httpx_mock: HTTPXMock
) -> None:
    ...
    httpx_mock.add_response(
        url="https://discourse.example.com/posts/456.json", method="PUT", json={"post": {"id": 456}}
    )
    httpx_mock.add_response(
        url="https://discourse.example.com/posts/456.json", method="PUT", json={"post": {"id": 456}}
    )
...
def test_message_exporter_batch_export(
    discourse_dbs: tuple[sqlite3.Connection, sqlite3.Connection], httpx_mock: HTTPXMock
) -> None:
    ...
    for i in range(5):
        httpx_mock.add_response(
            url="https://discourse.example.com/posts.json", method="POST", json={"id": 400 + i}
        )
