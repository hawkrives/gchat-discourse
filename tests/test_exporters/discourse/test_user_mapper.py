...
def test_user_mapper_creates_new_user(
    ...
    httpx_mock.add_response(
        url="https://discourse.example.com/users/alice_smith.json", method="GET", status_code=404
    )
    # Mock user creation
    httpx_mock.add_response(
...
def test_user_mapper_handles_existing_user(
...
def test_username_generation_short_names(
    discourse_dbs: tuple[sqlite3.Connection, sqlite3.Connection],
) -> None:
...
def test_user_mapper_handles_missing_user(
    discourse_dbs: tuple[sqlite3.Connection, sqlite3.Connection],
) -> None:
...
def test_user_mapper_handles_creation_failure(
    discourse_dbs: tuple[sqlite3.Connection, sqlite3.Connection], httpx_mock: HTTPXMock
) -> None:
